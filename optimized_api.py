...
def search_chunks_enhanced(index, query, embed_model, all_chunks, top_k=5):
    ...
    # Re-rank based on query term overlap
    ...
    for chunk, score in candidates:
        ...
        # Combine semantic similarity (inverted distance) with lexical overlap
    ...
    for attempt in range(retries):
        try:
            ...
            if "quota" in str(e).lower() or "rate" in str(e).lower():
                wait_time = min((2 ** attempt) * 10, 60)  # Cap at 60 seconds
...
# FastAPI app
app = FastAPI(title="LLM Document Processing API", version="2.0.0")
...
async def process_query(request: QueryRequest):
    ...
    try:
        ...
        if not request.documents:
            # You can set default document URLs here
...
if __name__ == "__main__":
    ...
    uvicorn.run("optimized_api:app", host="0.0.0.0", port=port, reload=False)
