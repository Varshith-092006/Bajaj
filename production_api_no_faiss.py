...
def build_vector_index_optimized(chunks, embed_model):
    if not chunks:
    ...
    index = NearestNeighbors(n_neighbors=5, algorithm='auto', metric='euclidean')
    index.fit(vectors)

    save_to_cache({'index': index, 'vectors': vectors}, cache_key, "vector_index")
    return index, vectors

def search_chunks_enhanced(index, query, embed_model, all_chunks, top_k=5, all_vectors=None):
    if not all_chunks or all_vectors is None:
        return []

    q_vec = embed_model.encode([query])
    distances, indices = index.kneighbors(q_vec, n_neighbors=min(top_k * 2, len(all_chunks)))

    candidates = []
    for j, i in enumerate(indices[0]):
        if i < len(all_chunks):
            candidates.append((all_chunks[i], distances[0][j]))
    ...
    for chunk, distance in candidates:
        ...
        combined_score = semantic_similarity * 0.7 + overlap_score * 0.3
...
def process_documents(documents: List[str]) -> tuple:
    ...
    index, vectors = build_vector_index_optimized(all_chunks, embed_model)
    return all_chunks, index, vectors
...
async def process_query(request: QueryRequest):
    ...
    all_chunks, index, all_vectors = process_documents(request.documents)

    relevant_chunks = search_chunks_enhanced(
        index, request.query, embed_model, all_chunks,
        top_k=request.top_k, all_vectors=all_vectors
    )
...
async def run_submission(request: RunRequest):
    ...
    for question in request.questions:
        try:
            relevant_chunks = search_chunks_enhanced(
                index, question, embed_model, all_chunks,
                top_k=5, all_vectors=all_vectors
            )
            context = "\n---\n".join(relevant_chunks)
...
if __name__ == "__main__":
    ...
    uvicorn.run("production_api_no_faiss:app", host="0.0.0.0", port=port, reload=False)
